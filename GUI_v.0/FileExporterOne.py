...
def SendFile(ser,route,textBox):
    ...
    for m in message:
        for car in m:
            ...
            if(i>config.FILE_SPLIT):
                ...
                sleep(2.5)        # Give some time to the arduino to send it (Upgrade would be improving delay time)
                binary_data = ""
                i = 0
                j+=1
            i+=1
    ...
    sleep(2)
    ser.write(bytes(str(config.ID)+config.ID_MARKER+"END"+config.END_MARKER, encoding="utf8"))    
    textBox.PB_step(0,1)
